...
python -u run_generation.py -m meta-llama/Llama-3.1-8B-Instruct --input-tokens 1024 --max-new-tokens 128 \
    --num-iter 8 --num-warmup 3 --batch-size 1 --device $DEVICE --token-latency --num-beams 1 --inductor \
    --use-static-cache --use-hf-code False --woq --woq-type rtn \
    --group-size 128 --quant-dtype uint4 --profile --attn-type flex_attention \
    >>"$RESULTS_DIR/llama31.uint4.fa.cpp.$DEVICE.profile.log" 2>&1
...
python -u run_generation.py -m meta-llama/Llama-3.1-8B-Instruct --input-tokens 1024 --max-new-tokens 128 \
    --num-iter 8 --num-warmup 3 --batch-size 1 --device $DEVICE --token-latency --num-beams 1 --inductor \
    --use-static-cache --use-hf-code False --woq --woq-type rtn \
    --group-size 128 --quant-dtype uint4 --profile --attn-type flex_attention --cpp-wrapper \
    >>"$RESULTS_DIR/llama31.uint4.fa.nocpp.$DEVICE.profile.log" 2>&1

python -u run_generation.py -m meta-llama/Llama-3.1-8B-Instruct --input-tokens 1024 --max-new-tokens 128 \
    --num-iter 8 --num-warmup 3 --batch-size 1 --device $DEVICE --token-latency --num-beams 1 --inductor \
    --use-static-cache --use-hf-code False --woq --woq-type rtn \
    --group-size 128 --quant-dtype uint4 --profile --attn-type sdpa --cpp-wrapper \
    >>"$RESULTS_DIR/llama31.uint4.sdpa.nocpp.$DEVICE.profile.log" 2>&1
